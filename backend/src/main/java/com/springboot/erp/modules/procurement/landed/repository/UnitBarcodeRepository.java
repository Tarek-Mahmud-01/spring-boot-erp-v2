...
public interface UnitBarcodeRepository extends JpaRepository<UnitBarcode, Long> {
    ...
    @Query("""
        ...
        where (:grnLineId is null or ub.grnLineId = :grnLineId)
          ...
          and (:search is null or lower(ub.barcode) like lower(concat('%', cast(:search as string), '%')))
        order by ub.id desc
        """)
    Page<UnitBarcode> search(@Param("grnLineId") String grnLineId,
                             ...
                             @Param("status") com.springboot.erp.modules.procurement.landed.domain.UnitBarcodeStatus status,
