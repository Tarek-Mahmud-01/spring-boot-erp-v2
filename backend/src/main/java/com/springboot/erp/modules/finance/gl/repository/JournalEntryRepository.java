...
public interface JournalEntryRepository extends JpaRepository<JournalEntry, Long> {
    ...
    @Query("""
        ...
        where e.companyId = :companyId
          ...
          and (:search is null
               or lower(e.voucherNumber) like lower(concat('%', cast(:search as string), '%'))
               or lower(e.reference) like lower(concat('%', cast(:search as string), '%'))
               or lower(e.narration) like lower(concat('%', cast(:search as string), '%')))
