package com.springboot.erp.modules.procurement.bills.repository;
...
public interface SupplierPaymentRepository extends JpaRepository<SupplierPayment, Long> {
    ...
    @Query("""
        ...
        where (:supplierId is null or p.supplierId = :supplierId)
          ...
          and (:search is null
               or lower(p.number) like lower(concat('%', cast(:search as string), '%'))
               or lower(p.invoiceReference) like lower(concat('%', cast(:search as string), '%'))
               or lower(p.referenceNo) like lower(concat('%', cast(:search as string), '%'))
               or lower(p.paymentMethodName) like lower(concat('%', cast(:search as string), '%')))
    ...
    @Query("""
        ...
        where p.poId = :poId
          and p.status in (com.springboot.erp.modules.procurement.bills.domain.SupplierPaymentStatus.APPROVED,
                           com.springboot.erp.modules.procurement.bills.domain.SupplierPaymentStatus.PARTIALLY_PAID,
                           com.springboot.erp.modules.procurement.bills.domain.SupplierPaymentStatus.PAID)
        """)
    ...
    @Query("""
        select p from SupplierPayment p
        where p.poId = :poId
          and p.status <> com.springboot.erp.modules.procurement.bills.domain.SupplierPaymentStatus.DRAFT
        """)
