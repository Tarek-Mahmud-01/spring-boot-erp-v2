...
public interface SupplierBillRepository extends JpaRepository<SupplierBill, Long> {
    ...
    @Query("""
        ...
        where (:supplierId is null or b.supplierId = :supplierId)
          ...
          and (:search is null
               or lower(b.number) like lower(concat('%', cast(:search as string), '%'))
               or lower(b.supplierBillNo) like lower(concat('%', cast(:search as string), '%'))
    ...
    @Query("""
        ...
        where b.poId = :poId
          and b.status not in (com.springboot.erp.modules.procurement.bills.domain.BillStatus.DRAFT,
                               com.springboot.erp.modules.procurement.bills.domain.BillStatus.CANCELLED)
        """)
