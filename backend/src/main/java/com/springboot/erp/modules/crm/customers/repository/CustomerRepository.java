package com.springboot.erp.modules.crm.customers.repository;
...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("""
        ...
        where (:companyId is null or c.companyId = :companyId)
          ...
          and (:query is null
               or lower(c.firstName) like cast(:query as string)
               or lower(c.lastName) like cast(:query as string)
               or lower(c.membershipId) like cast(:query as string)
               or lower(p.mobile) like cast(:query as string)
               or lower(p.email) like cast(:query as string))
        order by c.createdAt desc
        """)
    Page<Customer> search(@Param("companyId") String companyId,
                          @Param("status") com.springboot.erp.modules.crm.customers.domain.CustomerStatus status,
